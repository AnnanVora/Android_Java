...
public class DatePickerFragment extends DialogFragment implements DatePickerDialog.OnDateSetListener {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        if (args != null) {
            this.dialogID = args.getInt(DATE_PICKER_ID);
            title = args.getString(DATE_PICKER_TITLE);
            Date givenDate =  (Date) args.getSerializable(DATE_PICKER_DATE);
            if (givenDate != null) {
                cal.setTime(givenDate);
                Log.d(TAG, "onCreateDialog: given date = " + givenDate.toString());
            }
        }

        int yr = cal.get(GregorianCalendar.YEAR);
        int month = cal.get(GregorianCalendar.MONTH);
        int day = cal.get(GregorianCalendar.DAY_OF_MONTH);

        UnbuggyDatePickerDialog dpd = new UnbuggyDatePickerDialog(getContext(), this, yr, month, day);
        if (title != null) {
            dpd.setTitle(title);
        }
        return dpd;
    ...
    public void onAttach(@NonNull Context context) {
        ...
        if (!(context instanceof DatePickerDialog.OnDateSetListener)) {
            throw new ClassCastException(context.toString() + " must implement " + DatePickerDialog.OnDateSetListener.class.toString());
        }
    }
